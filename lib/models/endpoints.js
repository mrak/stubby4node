...
Endpoints.prototype.find = function(data, callback) {
  ...
  for (var id in this.db) {
    ...
    return found.call(this, matched, captures, headers, callback);
  }

  return callback(NO_MATCH);
};

function manipulateFileNameWithHeaders(fileName, headers) {
  var manipulatedFileName = fileName;
  var indexOfHeaderParamStartBracket = manipulatedFileName.indexOf('<');
  var indexOfHeaderParamEndBracket = manipulatedFileName.indexOf('>');

  // make sure we have enclosing brackets
  if(indexOfHeaderParamStartBracket > -1 && indexOfHeaderParamEndBracket > indexOfHeaderParamStartBracket) {
    var headerParam = manipulatedFileName.substring(indexOfHeaderParamStartBracket+1, indexOfHeaderParamEndBracket);
    var headerValue = headers[headerParam];

    if(headerValue) {
      manipulatedFileName = manipulatedFileName.replace('<' + headerParam + '>',headerValue);
    } else {
      manipulatedFileName = manipulatedFileName.replace('<' + headerParam + '>','default');
    }
  }

  return manipulatedFileName;
}
...
function found(endpoint, captures, headers, callback) {
  var filename, _ref;
  ...
  if (response.file != null) {
    ...
    filename = manipulateFileNameWithHeaders(filename, headers);
