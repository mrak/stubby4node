...
Endpoints.prototype.find = function(data, callback) {
  ...
  for (var id in this.db) {
    ...
    return found.call(this, matched, captures, userId, callback);
  }

  return callback(NO_MATCH);
};

function manipulateFileNameWithUserId(fileName, userId) {
  var manipulatedFileName = fileName;

  if(userId) {
    manipulatedFileName = manipulatedFileName.replace('parsed',userId);
  }
...
function found(endpoint, captures, userId, callback) {
  var filename, _ref;
  ...
  if (response.file != null) {
    ...
    filename = manipulateFileNameWithUserId(filename, userId);
