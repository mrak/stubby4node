  ...
  process.nextTick(function () {
    ...
    async.parallel({
      closeAdmin: function (cb) {
        if (self.adminPortal && self.adminPortal.address()) { self.adminPortal.close(cb); }
        else { cb(); }
      },
      closeStubs: function (cb) {
        if (self.stubsPortal && self.stubsPortal.address()) { self.stubsPortal.close(cb); }
        else { cb(); }
      },
      closeTls: function (cb) {
        if (self.tlsPortal && self.tlsPortal.address()) { self.tlsPortal.close(cb); }
        else { return cb(); }
      }
    }, callback);
  });
...
Stubby.prototype.post = function (data, callback) {
  ...
  var self = this;
  process.nextTick(function() {
    if (contract(data)) { callback(couldNotSave); }
    else { self.endpoints.create(data, callback); }
  });
};
...
Stubby.prototype.get = function (id, callback) {
  ...
  var self = this;
  process.nextTick(function () {
    if (typeof id === 'function') { self.endpoints.gather(callback); }
    else { self.endpoints.retrieve(id, callback); }
  });
};
...
Stubby.prototype.put = function (id, data, callback) {
  ...
  process.nextTick(function () {
    if (contract(data)) { callback(couldNotSave); }
    else { self.endpoints.update(id, data, callback); }
  });
};
