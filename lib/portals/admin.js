...
Admin.prototype.goDELETE = function(request, response) {
  ...
  function callback(err) {
      if (err) { self.notFound(response); }
      else { self.noContent(response); }
  }
...
Admin.prototype.goGET = function(request, response) {
  ...
  if (id) {
    var callback = function (err, endpoint) {
      if (err) { self.notFound(response); }
      else { self.ok(response, endpoint); }
    };

    return this.endpoints.retrieve(id, callback);
  } else {
    var callback = function (err, data) {
        if (data.length === 0) { _this.noContent(response); }
        else { self.ok(response, data); }
    };
...
Admin.prototype.processPUT = function(id, data, response) {
  ...
  catch (e) { return this.badRequest(response); }

  var errors = this.contract(data);
